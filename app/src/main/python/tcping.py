    ...
    try:
        ...
        if url != None:
            if validIP(url) == True:
                ...
                hipAddr = socket.gethostbyname(str(url))
                if validIP(hipAddr) == True:
                    settings['ip'] = str(hipAddr)
                else:
                    print("Can't resolve Hostname " + str(url) + "[" + str(hipAddr) + "]", "DNS")
                    sys.exit(1)
    except Exception as ErrMs:
        err(ErrMs, "GetOp")
        sys.exit(1)
...
###
def my_ping(url, log, latitude, longitude):
    settings['log'] = log
    msg("Host Name: " + url)
    msg("Location: [latitude:longitude] " + "[" + latitude + ":" + longitude + "]")
...
if __name__ == "__main__":
    my_ping("video-edge-87b0fa.jkt02.abs.hls.ttvnw.net", "loga.txt", "123", "423")